...
def transform_and_create_mart(**kwargs):
    ...
    mart_df['last_seen_date'] = mart_df['last_seen_date'].apply(lambda x: x.isoformat() if pd.notnull(x) else None)
    mart_df['report_updated_at'] = datetime.now().isoformat()
...
with DAG(
    ...
    extract_crm_task = PythonOperator(
        task_id='extract_crm_task', python_callable=extract_crm_data_from_csv
    )
    extract_telemetry_task = PythonOperator(
        task_id='extract_telemetry_task', python_callable=extract_telemetry_data_from_csv
    )
    transform_task = PythonOperator(
        task_id='transform_task', python_callable=transform_and_create_mart
    )
    load_task = PythonOperator(
        task_id='load_task', python_callable=load_mart_to_olap
    )
